...
class gsv_wrapper {
   public:
    ...
    void verify(size_t instance_count, instance_t *instances, int32_t *results) {
        CUDA_CHECK(cudaMemcpy(d_instances, instances, sizeof(instance_t) * instance_count, cudaMemcpyHostToDevice));

        kernel_sig_verify<params>
            <<<(instance_count + IPB - 1) / IPB, TPB>>>(report, d_instances, instance_count, sm2, d_results);
...
int main(int argc, char **argv) {
    typedef gsv_params_t<16, 512> params;
    typedef typename gsv_t<params>::instance_t instance_t;

    gsv_wrapper<params> gsv;
    instance_t *instances;
    int32_t *results;

    int count = 1024;

    instances = (instance_t *)malloc(sizeof(instance_t) * count);
    results = (int32_t *)malloc(sizeof(int32_t) * count);

    for (int i = 0; i < count; i++) {
        set_words(instances[i].r._limbs, "23B20B796AAAFEAAA3F1592CB9B4A93D5A8D279843E1C57980E64E0ABC5F5B05", params::BITS / 32);
        set_words(instances[i].s._limbs, "E11F5909F947D5BE08C84A22CE9F7C338F7CF4A5B941B9268025495D7D433071", params::BITS / 32);
        set_words(instances[i].e._limbs, "10D51CB90C0C0522E94875A2BEA7AB72299EBE7192E64EFE0573B1C77110E5C9", params::BITS / 32);
        // set_words(instances[index].key_x._limbs, "D5548C7825CBB56150A3506CD57464AF8A1AE0519DFAF3C58221DC810CAF28DD",
        //           params::BITS / 32);
        // set_words(instances[index].key_y._limbs, "921073768FE3D59CE54E79A49445CF73FED23086537027264D168946D479533E",
        //           params::BITS / 32);
    ...
    gsv.verify(count, instances, results);
